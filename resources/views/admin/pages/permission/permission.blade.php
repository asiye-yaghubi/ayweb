                    ...
                    <table class="table table-bordered table-striped table-hover js-basic-example dataTable align-center" id="data-table">
                        ...
                        <tbody id="values-crud">
                            ...
                            <tr id="value_id_{{ $u_info->id }}">
                                ...
                                <td>{{ $u_info->slug }}</td>
                                {{-- <td>{{ $u_info->created_at->toFormattedDateString() }}</td> --}}
                                <td>
...
@section('extra-script-footer')
    ...
    <script>
        ...
        // $('body').on('click', '#delete-value', function () {
        //       var value_id = $(this).data("id");
        //       if(confirm("{{__('home.are you sure ?!')}}")) {
        //       $.ajax({
        //           type: "DELETE",
        //           url: "{{ url('manage/permission')}}"+'/'+value_id,
        //           success: function (data) {
       ...
       if ($("#valueForm").length > 0) {
            ...
            var value = '<tr id="value_id_' + data.permission.id + '"><td>' + data.permission.id + '</td><td> <a id="show-value" data-id="' + data.permission.id + '" href="javascript:void(0)' + data.permission.id + '">' + data.permission.title + '</a></td><td>'+ data.permission.slug + '</td>';                                                      
            value += '<td><a href="javascript:void(0)" id="edit-value" data-id="' + data.permission.id + '" class="btn bg-blue btn-circle waves-effect waves-circle waves-float"><i class="material-icons">mode_edit</i></a> &nbsp;';
            value += ' <a href="javascript:void(0)" id="delete-value" data-id="' + data.permission.id + '" class="btn bg-red btn-circle waves-effect waves-circle waves-float"><i class="material-icons">delete_forever</i></a></td></tr>';
